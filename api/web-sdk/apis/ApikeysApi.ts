...
export interface DeleteApiKeyRequest {
    /**
     * Identifier of the API key, as returned by `POST /api-keys`. Not the key secret.
     */
    keyId: string;
}
...
export class ApikeysApi extends runtime.BaseAPI {
    ...
    /**
     * Revoke a key for good. It stops working within about 30 seconds, after the authorization cache expires.  Only the wallet that created a key can delete it. A deleted key cannot be brought back, and a key\'s permissions can never be edited — to change what a key may do, create a new one and delete this one.  Requires the `api-keys.delete` permission. 
     * Delete an API key
     */

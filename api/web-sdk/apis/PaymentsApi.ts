...
export interface ListDepositsRequest {
    ...
    sortBy?: string;
    /**
     * Return only the deposit made by this transaction.
     */
    txHash?: string;
    /**
     * Return only deposits of this token.
     */
    token?: DepositToken;
    /**
     * Return only deposits of the token at this contract address.
     */
    tokenAddress?: string;
    /**
     * Return only deposits in this state.
     */
...
export class PaymentsApi extends runtime.BaseAPI {
    ...
    /**
     * Return the on-chain deposits observed for your organization, newest first.  Every row is a payment made on chain: who sent it, how much, in which token, and the transaction that carried it. A deposit shows up here shortly after its transaction is mined. A `pending` deposit has been queued for balance crediting, `completed` is reserved for a downstream acknowledgement, and `failed` means the deposited asset is not supported.  You only ever see your own organization\'s deposits. 
     * List deposits
     */

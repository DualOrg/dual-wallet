...
/**
 * Notification the organization sends when this action completes. One message
 * per action, addressed to the owner of the action's first object after the
 * change — the new owner on a transfer or claim, the unchanged one otherwise.
 * 
 * The message describes that first object. A bulk mint sends one email, with
 * object_count carrying how many were created.
 * 
 * An owner with no wallet, no email address, or an object held by another
 * object receives nothing; the action still succeeds.
 * 
 * @export
 * @interface ActionNotifications
 */
export interface ActionNotifications {
    /**
     * Whether this action sends a notification
     */
    enabled: boolean;
    /**
     * Notification template used to render the message. It must belong to the
     * same organization as this smart-object template. Omit it to fall back to
     * the organization's default template for the custom action type.
     * 
     */
    templateId?: string;
}
...
/**
 * Check if a given object implements the ActionNotifications interface.
 */
export function instanceOfActionNotifications(value: object): value is ActionNotifications {
    if (!('enabled' in value) || value['enabled'] === undefined) return false;
    return true;
}

export function ActionNotificationsFromJSON(json: any): ActionNotifications {
    return ActionNotificationsFromJSONTyped(json, false);
}

export function ActionNotificationsFromJSONTyped(json: any, ignoreDiscriminator: boolean): ActionNotifications {
    if (json == null) {
        return json;
    }
    return {
...
export function ActionNotificationsToJSON(json: any): ActionNotifications {
    return ActionNotificationsToJSONTyped(json, false);
}

export function ActionNotificationsToJSONTyped(value?: ActionNotifications | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

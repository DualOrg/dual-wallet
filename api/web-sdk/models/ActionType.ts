...
/**
 * What the message is for. It selects the template a send falls back to, and it
 * is how you filter your message history.
 * 
 * | Value | Sent when |
 * | --- | --- |
 * | `register` | A wallet was created |
 * | `login` | A sign-in needs confirming |
 * | `reset` | A password reset was requested |
 * | `verify` | An email or phone number needs verifying |
 * | `otp` | A one-time code was requested |
 * | `org_invite` | Someone was invited to an organization |
 * | `custom` | Your own message. Use this for anything you send yourself |
 * | `internal` | Platform-internal mail |
 * 
 * @export
 */

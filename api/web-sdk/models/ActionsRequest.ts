...
/**
 * One action. Set exactly one of the fields below — `mint` to create objects,
 * `transfer` to hand one over, `redeem` to use one up, and so on. Each carries
 * its own details.
 * 
 * @export
...
export interface ActionsRequest {
    /**
     * The template's own name for this action, when it has one — `check-in`
     * instead of `redeem`. Send it when the template defines an alias and the
     * action is addressed by it.
     * 
     */
    alias?: string;
    /**
     * The secret behind a permit, when the action is being run under another
     * account's authority rather than your own.
     * 
     */
    permitSecret?: string;
    /**

...
/**
 * One action carried by a batch.
 * @export
 * @interface AffectedActionLog
 */
export interface AffectedActionLog {
    /**
     * The action's id. Look it up with `GET /ebus/action-logs?id=`.
     * 
     */
    id: string;
    /**
     * The action's hash, as committed to by the batch.
     */
    hash: string;
    /**
     * The action that ran, for example `mint` or `transfer`.
     */
    name: string;
    /**
     * The template's own name for the action, when it defines one.
     */
    alias?: string;
}

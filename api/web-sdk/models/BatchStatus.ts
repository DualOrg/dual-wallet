...
/**
 * Where a batch is in the sequencer pipeline.
 * 
 * | Value | Meaning |
 * | --- | --- |
 * | `building` | Actions selected, state and hashes being computed |
 * | `requesting` | Proof input serialized, proof requested |
 * | `proving` | Waiting for the proof |
 * | `storing` | Proof input being pinned for public verification |
 * | `anchoring` | Commitment being written on chain |
 * | `nfts` | On-chain NFT side effects being applied |
 * | `settling` | Waiting for the challenge window to pass |
 * | `finalized` | Anchored and settled. Terminal |
 * | `failed` | The pipeline gave up. Terminal |
 * 
 * @export
 */

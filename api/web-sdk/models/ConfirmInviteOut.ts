...
/**
 * A signed-in session for the organization the invitation was for.
 * 
 * @export
...
export interface ConfirmInviteOut {
    /**
     * Send this on every request, as `Authorization: Bearer <access_token>`. It
     * lasts about fifteen minutes.
     * 
     */
    accessToken: string;
    /**
     * Exchange this at `POST /auth/refresh-token` for a new pair when the access
     * token runs out. Keep only the newest one.
     * 
     */
    refreshToken: string;
}

...
/**
 * Extra values merged into the notification body, used only when the action's
 * template turns notifications on.
 * 
 * These values are not covered by the EIP-712 signature and are not recorded in
 * the action log, so they carry presentation detail only. The service-supplied
 * keys — object_id, name, description, image_url, action and action_id — always
 * overwrite a caller value of the same name.
 * 
 * @export
 * @interface ExecuteNotifications
 */
export interface ExecuteNotifications {
    /**
     * Free-form values passed to the notification template
     */
    values?: { [key: string]: any; };
}

/**
 * Check if a given object implements the ExecuteNotifications interface.
 */
export function instanceOfExecuteNotifications(value: object): value is ExecuteNotifications {
    return true;
}

export function ExecuteNotificationsFromJSON(json: any): ExecuteNotifications {
    return ExecuteNotificationsFromJSONTyped(json, false);
}

export function ExecuteNotificationsFromJSONTyped(json: any, ignoreDiscriminator: boolean): ExecuteNotifications {
    if (json == null) {
        return json;
    }
    return {
...
export function ExecuteNotificationsToJSON(json: any): ExecuteNotifications {
    return ExecuteNotificationsToJSONTyped(json, false);
}

export function ExecuteNotificationsToJSONTyped(value?: ExecuteNotifications | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

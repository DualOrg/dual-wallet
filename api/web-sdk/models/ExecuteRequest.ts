...
import type { ExecuteNotifications } from './ExecuteNotifications';
import {
    ExecuteNotificationsFromJSON,
    ExecuteNotificationsFromJSONTyped,
    ExecuteNotificationsToJSON,
    ExecuteNotificationsToJSONTyped,
} from './ExecuteNotifications';

/**
 * An action to run: what to do, the wallet's action number, and the signature
 * that authorises it.
 * 
 * @export
...
export interface ExecuteRequest {
    /**
     * What to do. Set exactly one of the actions listed below.
     * 
     */
    action: ActionsRequest;
    /**
     * The wallet's current action number, from `POST /ebus/prepare` or
     * `GET /ebus/nonce`. It has to be the current one — an action signed with a
     * number that has since moved on is refused.
     * 
     */
    nonce: number;
    /**
    ...
    /**
     * The signature that authorises this action, and which kind of signature it
     * is.
     * 
     * Whatever the kind, what gets signed is the same thing: the value
     * `POST /ebus/prepare` returned for this exact action and number. For a
     * passkey, that value is the WebAuthn challenge — pass it straight to
     * `navigator.credentials.get()`.
     * 
     * Leave this out for a custodial account, where Dual holds the key and signs
     * on its behalf.
     * 
     */
    auth?: AuthBundle;
...
export function ExecuteRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ExecuteRequest {
    ...
    return {
        ...
        'nonce': json['nonce'],
        'notifications': json['notifications'] == null ? undefined : ExecuteNotificationsFromJSON(json['notifications']),
...
export function ExecuteRequestToJSONTyped(value?: ExecuteRequest | null, ignoreDiscriminator: boolean = false): any {
    ...
    return {
        ...
        'nonce': value['nonce'],
        'notifications': ExecuteNotificationsToJSON(value['notifications']),

...
/**
 * One design, for one place an object appears. Designs are only ever shown to
 * your own organization; everyone else sees the finished result.
 * 
 * @export
 * @interface FaceView
 */
export interface FaceView {
     ...
     * | --- | --- |
     * | `card` | A tile in a list or wallet |
     * | `detail` | The object's own page |
     * | `share` | The preview card in a chat or social post |
     * | `default` | An older catch-all, used when no variant fits |
     * 
     */
    variant: FaceViewVariantEnum;
    /**
     * What this design produces. An inline design has to be `text/html` or
     * `image/svg+xml`; a page of your own can be anything.
     * 
     */
    mediaType: FaceViewMediaTypeEnum;
    /**
     * The design itself, as HTML or SVG, up to 1 MB. Write
     * `{{.metadata.name}}`, `{{.owner}}` or `{{.custom.tier}}` wherever the
     * object's own details should appear.
     * 
     * A view needs either this or `url`, never both.
     * 
     */
    content?: string;
    /**
     * A page of your own to show instead of a design, over `https`. We add
     * `object_id` and `variant` to the address so your page knows what it is
     * showing, and keep any query parameters you already have.
     * 
     * A view needs either this or `content`, never both.
     * 
     */
    url?: string;
    /**
     * The shape this design is drawn in, so apps can leave the right amount of
     * room before it loads.
     * 
     */
    aspectRatio?: string;
    /**
     * Whether the design responds to taps and clicks, rather than just being
     * looked at.
     * 
     */
    interactive?: boolean;
    /**
     * Settings your design or page can read while it draws.
     * 
     * Treat everything here as public: it is handed to the page along with the
     * object. Never put keys, tokens or anything secret in it.
     * 
     */

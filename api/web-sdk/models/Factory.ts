...
/**
 * How many objects a template allows, and when they may be created.
 * 
 * @export
 * @interface Factory
 */
export interface Factory {
    /**
     * The most objects that may ever exist. Cannot later be lowered below the
     * number already created.
     * 
     */
    maxSupply: number;
    /**
     * How many objects have been created from this template so far. Counted for
     * you; a value sent here is ignored.
     * 
     */
    readonly mintedCount: number;
    /**
     * When objects may start being created. A time in the past means "from
     * now".
     * 
     */
    startTime?: Date;
    /**
     * When creating stops. Leave it out to keep it open. Closing this window is
     * the safe way to retire a template without disturbing the objects already
     * out there.
     * 
     */
    endTime?: Date;
    /**
     * Addresses allowed to create objects, when creation is restricted.
     */
...
export function FactoryToJSONTyped(value?: Omit<Factory, 'mintedCount'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

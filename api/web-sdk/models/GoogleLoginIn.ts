...
/**
 * The Google credential to exchange for a Dual session. Send the ID token
 * returned in the Google Identity Services callback, not an OAuth access token
 * or authorization code.
 * 
 * @export
 * @interface GoogleLoginIn
 */
export interface GoogleLoginIn {
    /**
     * The JWT from the Google Identity Services callback's `credential` field.
     */
    idToken: string;
    /**
     * Organization whose account should be used. Omit this field to sign in to
     * the open network scope. The same Google account may have a separate Dual
     * wallet in each scope.
     * 
     */
    organizationId?: string;
    /**
     * Preferred language for a newly created wallet. Ignored for a returning wallet.
     */
    language?: Language;
}
...
/**
 * Check if a given object implements the GoogleLoginIn interface.
 */
export function instanceOfGoogleLoginIn(value: object): value is GoogleLoginIn {
...
export function GoogleLoginInFromJSON(json: any): GoogleLoginIn {
    return GoogleLoginInFromJSONTyped(json, false);
}

export function GoogleLoginInFromJSONTyped(json: any, ignoreDiscriminator: boolean): GoogleLoginIn {
    if (json == null) {
        return json;
    }
    return {
...
export function GoogleLoginInToJSON(json: any): GoogleLoginIn {
    return GoogleLoginInToJSONTyped(json, false);
}

export function GoogleLoginInToJSONTyped(value?: GoogleLoginIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

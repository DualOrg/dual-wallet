...
/**
 * A signed-in session, and the account it belongs to.
 * @export
 * @interface LoginOut
 */
export interface LoginOut {
    ...
    wallet: Wallet;
    /**
     * Send this on every request, as `Authorization: Bearer
     * <access_token>`. It lasts about fifteen minutes.
     * 
     */
    accessToken: string;
    /**
     * Exchange this at `POST /auth/refresh-token` for a new pair when the
     * access token runs out. Each one works once, so always keep the
     * newest.
     * 
     */
    refreshToken: string;

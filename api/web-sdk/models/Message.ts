...
/**
 * One message's delivery record: who it went to, over which channel, and how it
 * ended. The rendered subject and body are not returned.
 * 
 * @export
 * @interface Message
 */
export interface Message {
    /**
     * Identifier of the message, as returned by `POST /messages/send`.
     */
    id: string;
    /**
     * The template this message was rendered from. Absent when the body was
     * provided directly or when no template matched.
     * 
     */
    templateId?: string;
    /**
    ...
    language: Language;
    /**
     * Where this message is in its delivery life cycle.
     */
    status: MessageStatus;
    /**
     * The requested send time. Scheduling is not in service, so this is only
     * ever a past instant.
     * 
     */
    scheduledAt?: Date;
    /**
     * Why the last attempt failed, as reported by the delivery provider. Absent
     * unless the status is `failed` or `retrying`.
     * 
     */
    error?: GeneralError;
    /**

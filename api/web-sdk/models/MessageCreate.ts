...
/**
 * A message to render and send. Either name a template, or provide `content`
 * and `content_type` directly.
 * 
 * @export
...
export interface MessageCreate {
    /**
     * Render with this specific template of yours. Omit it to let the action
     * type, language and channel select one.
     * 
     */
    templateId?: string;
    /**
    ...
    /**
     * Delivery channel. Set it explicitly; leaving it out makes the service
     * guess from the shape of `to`.
     * 
     */
    system?: System;
    /**
     * The recipient. An email address, since email is the only channel in service.
     */
    to: string;
    /**
     * Variables for the template. `{{.first_name}}` in the template reads
     * `first_name` here. Ignored when you provide `content` yourself.
     * 
     */
    values?: object;
    /**
     * When to send. Scheduling is not yet available: a time in the future is
     * refused with `422`, rather than being sent immediately.
     * 
     */
    scheduledAt?: Date;
    /**
    ...
    language?: Language;
    /**
     * Subject line, when you are providing the content yourself. Ignored when a
     * template supplies one.
     * 
     */
    title?: string;
    /**
     * The body, when you do not want a template. Providing it skips template
     * resolution entirely, so `values` are not substituted — send the final
     * text.
     * 
     */
    content?: string;
    /**
     * Required when you provide `content` directly.
     */

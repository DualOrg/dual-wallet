...
import type { MessageQuotaBucket } from './MessageQuotaBucket';
import {
    MessageQuotaBucketFromJSON,
    MessageQuotaBucketFromJSONTyped,
    MessageQuotaBucketToJSON,
    MessageQuotaBucketToJSONTyped,
} from './MessageQuotaBucket';

/**
 * What is left of the sending allowance for the caller's organization. A
 * bucket with no cap configured is left out.
 * 
 * @export
...
export interface MessageQuota {
    /**
     * Length of the rolling window. A message stops counting once it is
     * this old, so an allowance recovers gradually rather than resetting.
     * 
     */
    windowSeconds: number;
    /**
     * What the organization has left.
     */
    organization?: MessageQuotaBucket;
    /**
     * What the address in `recipient` has left. Absent unless that
     * parameter was sent.
     * 
     */
    recipient?: MessageQuotaBucket;
}

/**
 * Check if a given object implements the MessageQuota interface.
 */
export function instanceOfMessageQuota(value: object): value is MessageQuota {
    if ((!('windowSeconds' in (value as Record<string, any>)) && !('window_seconds' in (value as Record<string, any>))) || ((value as Record<string, any>)['windowSeconds'] === undefined && (value as Record<string, any>)['window_seconds'] === undefined)) return false;
    return true;
}

export function MessageQuotaFromJSON(json: any): MessageQuota {
    return MessageQuotaFromJSONTyped(json, false);
}

export function MessageQuotaFromJSONTyped(json: any, ignoreDiscriminator: boolean): MessageQuota {
    if (json == null) {
        return json;
    }
    return {
        ...
        'organization': json['organization'] == null ? undefined : MessageQuotaBucketFromJSON(json['organization']),
        'recipient': json['recipient'] == null ? undefined : MessageQuotaBucketFromJSON(json['recipient']),
    };
}

export function MessageQuotaToJSON(json: any): MessageQuota {
    return MessageQuotaToJSONTyped(json, false);
}

export function MessageQuotaToJSONTyped(value?: MessageQuota | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    return {
        ...
        'organization': MessageQuotaBucketToJSON(value['organization']),
        'recipient': MessageQuotaBucketToJSON(value['recipient']),
    };
}

...
/**
 * The body of every failed request, whatever the status code.
 * 
 * Branch on the HTTP status and on `code`. `message` is written for a human
 * reading a log and may change at any time.
 * 
 * @export
...
export interface ModelError {
     ...
     * | 10 | Aborted — the request lost a race and may be retried |
     * | 12 | Unimplemented |
     * | 13 | Internal |
     * | 14 | Unavailable — a required dependency could not accept the request |
     * | 16 | Unauthenticated |
     * 
     */
    code: number;
    ...
    message: string;
    /**
     * Optional structured context, present on validation failures. Its shape
     * depends on the failure and is not part of the stable contract.
     * 
     */
    details?: object;
}

...
/**
 * A ready-made way to show an object: where to load it from, what shape it
 * draws in, and whether it responds to taps. The design behind it is never
 * included.
 * 
 * @export
...
export interface ObjectDisplay {
    ...
    /**
     * What `href` returns — a page, an image, and so on.
     */
    mediaType: string;
    /**
     * Where to load the object's appearance from. Either an address on this
     * API or, where the organization hosts its own page, an address of
     * theirs with `object_id` and `variant` added.
     * 
     */
    href: string;
    /**
     * Changes whenever the design changes. Use it in a cache key so a new
     * look reaches people immediately.
     * 
     */
    revision: string;
    /**
     * The shape it draws in, so you can leave the right amount of room
     * before it loads.
     * 
     */
    aspectRatio?: string;
    /**
     * Whether it responds to taps and clicks, rather than just being looked at.
     */
    interactive: boolean;
    /**
     * Settings the design publishes for whoever displays it. Always
     * public — never anything secret.
     * 
     */

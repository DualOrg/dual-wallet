...
/**
 * One thing a role allows: an area of the API, and what may be done in it.
 * 
 * Areas do not nest. A permission on `organizations` grants nothing on
 * `organizations.members`; each is granted on its own.
 * 
 * @export
...
export interface Permission {
    /**
     * The area this permission covers.
     * 
     * | Area | Covers |
     * | --- | --- |
     * | `api-keys` | Creating and revoking API keys |
     * | `ebus.actions` | Preparing and running actions |
     * | `ebus.action-logs` | Reading what has been done |
     * | `faces` | Designing how objects look |
     * | `notifications.messages` | Sending email and reading what was sent |
     * | `notifications.templates` | Writing email templates |
     * | `objects` | Reading objects |
     * | `objects.state-changes` | Reading an object's history |

...
/**
 * Everything you need in order to sign an action.
 * @export
 * @interface PrepareExecuteOut
 */
export interface PrepareExecuteOut {
    /**
     * The wallet's current action number. Send it back unchanged with the
     * action.
     * 
     */
    nonce: number;
    /**
     * The exact value to sign, base64url-encoded. For a passkey, pass it to
     * `navigator.credentials.get()` as the `challenge`.
     * 
     */
    challenge: string;
}

...
export interface RefreshTokenOut {
    /**
     * The new access token. Send it on every request from now on.
     * 
     */
    accessToken: string;
    /**
     * The new refresh token, which replaces the one you sent. Store it and
     * throw the old one away — reusing a spent refresh token ends the
     * session everywhere.
     * 
     */
    refreshToken?: string;

...
/**
 * A staking series, bucketed over time.
 * @export
 * @interface StakingOperationsStats
 */
export interface StakingOperationsStats {
    /**
     * On `/operations/stats`, the number of staking operations in the window.
     * On `/operations/total`, the number of points in `data`.
     * 
     */
    total: number;
    /**
     * The buckets, oldest first. `when` is the start of the bucket, `key` the
     * operation type when the series is split by type, `count` the number of
     * operations and `amount` their sum in wei.
     * 
     */

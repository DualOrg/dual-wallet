...
/**
 * A stored file and its metadata. The bytes live with the provider named in
 * `provider`; this record is the organization's index of them.
 * 
 * @export
...
export interface StorageAsset {
    ...
    id: string;
    /**
     * The wallet that uploaded the file.
     */
    walletId: string;
    /**
    ...
    name: string;
    /**
     * Free-form label set at upload time. Defaults to `default`.
     */
    tag: string;
    /**
     * Provider URL of the object. For a private asset it is not directly
     * readable — use `GET /assets/{assetId}/download` for a signed URL. For an
     * IPFS asset it is the public gateway URL.
     * 
     */
    url: string;
    /**
     * Where the object sits with its provider: `<organization>/<folder>/<key>`
     * on Google Cloud Storage, or the pinned path whose first segment is the CID
     * on IPFS.
     * 
     */
    path: string;
    ...
    folder: FolderType;
    /**
     * Whether the object went to the public bucket. A public asset is readable
     * from its `url`; a private one is reachable only through a signed download
     * URL.
     * 
     */
    isPublic?: boolean;
    /**
     * Media type detected by the server from the file's own bytes, not from the
     * `Content-Type` the client declared.
     * 
     */
    type: string;
    /**
     * Content digest reported by the provider — the MD5 of the object on Google
     * Cloud Storage, the CID on IPFS.
     * 
     */
    hash?: string;
    /**

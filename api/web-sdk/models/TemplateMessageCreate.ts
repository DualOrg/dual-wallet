...
/**
 * A message template to create. `title` and `content` are Go templates; both are
 * parsed before the template is stored.
 * 
 * @export
...
export interface TemplateMessageCreate {
    ...
    /**
     * Subject line. May use variables: `Your ticket {{.ticket}} is ready`.
     * 
     */
    title: string;
    /**
     * Free-form grouping label of your own.
     */
    project?: string;
    /**
     * The body, at most 4096 characters. HTML is escaped per `content_type`, so
     * an `html` template's variables are safe to interpolate into markup.
     * 
     */
    content: string;
    /**
    ...
    /**
     * Default values for variables this template uses. They are rendered against
     * the send-time values first, so a default may itself refer to another
     * variable. A value supplied at send time wins.
     * 
     */

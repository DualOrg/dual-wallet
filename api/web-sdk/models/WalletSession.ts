...
/**
 * One place this account is signed in — one browser, or one device.
 * 
 * Not to be confused with `Session`, which is a key the account has authorised
 * to act on chain on its behalf. This one only keeps a user signed in.
 * 
 * @export
...
export interface WalletSession {
     ...
     * `DELETE /wallets/sessions/{sessionId}` to sign that device out.
     * 
     */
    id: string;
    /**
     * `true` for the session you are calling from — the one to mark as "this
     * device".
     * 
     */
    current: boolean;
    /**
     * What the browser or app said it was when the session was opened. Turn it
     * into a friendly device name for display — but treat it as a hint, not a
     * fact: it is not a name the person chose, and anything can be sent here.
     * 
     */
    userAgent?: string;
    /**
     * The organization this session is working in. Absent for an account that is
     * not tied to one.
     * 
     */
    orgId?: string;
    /**
     * When they signed in on this device.
     */
    whenCreated: Date;
    /**
     * When this device was last active — the best signal for spotting a session
...
/**
 * Check if a given object implements the WalletSession interface.
 */
export function instanceOfWalletSession(value: object): value is WalletSession {
...
export function WalletSessionFromJSON(json: any): WalletSession {
    return WalletSessionFromJSONTyped(json, false);
}

export function WalletSessionFromJSONTyped(json: any, ignoreDiscriminator: boolean): WalletSession {
    if (json == null) {
        return json;
    }
    return {
        ...
        'lastUsed': (json['last_used'] == null ? json['last_used'] : new Date(json['last_used'])),
        'expires': (json['expires'] == null ? json['expires'] : new Date(json['expires'])),
    };
}

export function WalletSessionToJSON(json: any): WalletSession {
    return WalletSessionToJSONTyped(json, false);
}

export function WalletSessionToJSONTyped(value?: WalletSession | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

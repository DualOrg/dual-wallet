...
/**
 * A subscription that sends matching action events to an HTTPS endpoint.
 * @export
 * @interface Webhook
 */
export interface Webhook {
    /**
     * Identifier of the webhook. Sent with every delivery as `X-Webhook-ID`.
     */
    id: string;
    /**
     * Your label for this subscription.
     */
    name: string;
    /**
     * The wallet that created this webhook.
     */
    walletId: string;
    /**
    ...
    url: string;
    /**
     * Whether deliveries are being made. Set to `false` automatically after
     * repeated delivery failures; re-enable it once the endpoint is healthy.
     * 
     */
    isActive: boolean;
    /**
     * Template-scoped match rules. At least one rule is required, and the
     * webhook is notified when any rule matches.
     * 
     */

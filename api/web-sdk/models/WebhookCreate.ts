...
/**
 * A webhook subscription to create.
 * @export
 * @interface WebhookCreate
 */
export interface WebhookCreate {
    /**
     * Your label for this subscription. Shown in the dashboard and in list responses.
     */
    name: string;
    /**
     * Where deliveries are sent. Must be `https` on the default port, 80 or
     * 443 and with no credentials in the URL. Literal private, loopback and
     * link-local addresses are rejected immediately. Hostnames are resolved
     * and checked again for every delivery so DNS changes cannot redirect a
     * request into a private network.
     * 
     */
    url: string;
    /**
     * Whether deliveries start immediately. Defaults to `true` when omitted.
     */
    isActive?: boolean;
    /**
     * Template-scoped match rules. Rules are ORed; within one rule the template,
     * action and signer restrictions are ANDed. Each template may appear once.
     * 
     */
...
export function WebhookCreateFromJSONTyped(json: any, ignoreDiscriminator: boolean): WebhookCreate {
    ...
    return {
        ...
        'rules': ((json['rules'] as Array<any>).map(WebhookRuleFromJSON)),
    };
}
...
export function WebhookCreateToJSONTyped(value?: WebhookCreate | null, ignoreDiscriminator: boolean = false): any {
    ...
    return {
        ...
        'rules': ((value['rules'] as Array<any>).map(WebhookRuleToJSON)),
    };
}

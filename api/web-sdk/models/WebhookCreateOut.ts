...
/**
 * The created webhook, with its signing key.
 * @export
 * @interface WebhookCreateOut
 */
export interface WebhookCreateOut {
    /**
     * Identifier of the new webhook.
     */
    id: string;
    /**
     * The HMAC key for verifying deliveries, as a hex string. Decode it
     * from hex before using it as the HMAC key.
     * 
     * It is returned here and nowhere else: no read endpoint echoes it
     * back and there is no rotation endpoint. Store it now.
     * 
     */
    signingKey: string;
}

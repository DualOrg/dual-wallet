...
/**
 * One delivery attempt record: the body that was sent to your endpoint, the
 * signature it carried, and how it ended.
 * 
 * @export
...
export interface WebhookEvent {
    /**
     * Identifier of the delivery, sent as the `X-Event-ID` header. It is stable
     * across retries and across a redelivery, so it is the right key for making
     * your handler idempotent.
     * 
     */
    id: string;
    /**
     * The subscription that matched, sent as the `X-Webhook-ID` header.
     */
    webhookId: string;
    /**
     * 
    ...
    type: WebhookType;
    /**
     * Exactly what was sent to your endpoint, as it was sent. Check signatures
     * against this text as it arrives, never against a re-encoded copy of it.
     * 
     */
    payload: string;
    /**
     * The `X-Webhook-Signature` that went out with the most recent attempt. It
     * is calculated afresh each time, so a retry carries a different one.
     * 
     */
    signature: string;
    /**
     * The endpoint this delivery was sent to, as configured at the time.
     */
    url: string;
    /**
     * 
    ...
    /**
     * Why the last attempt failed, including the status code and a snippet of
     * the response body. Absent while the delivery is succeeding.
     * 
     */
    error?: GeneralError;
    /**
     * When the delivery last changed state.
     */
    whenModified?: Date;
    /**
     * When the action was matched and the delivery was created.
     */
    whenCreated: Date;
}

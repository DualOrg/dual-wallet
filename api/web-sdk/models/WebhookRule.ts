...
/**
 * One template-scoped webhook rule. The template must match, and any non-empty
 * action or address list must also match. Empty `actions` means every action on
 * this template; empty `addresses` means every action signer. A webhook matches
 * when any one of its rules matches.
 * 
 * @export
...
export interface WebhookRule {
    /**
     * The template whose actions this rule watches.
     */
    templateId: string;
    /**
     * Action names to match. Empty or omitted means every action on the template.
     */
    actions?: Array<string>;
    /**
     * Action signer addresses to match. Empty or omitted means every signer.
     * Values are accepted case-insensitively and returned in lowercase form.
     * 
     */

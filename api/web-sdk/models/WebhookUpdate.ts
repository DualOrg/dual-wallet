...
/**
 * The fields of a webhook to change. Every field is optional; only what you send
 * is changed.
 * 
 * @export
...
export interface WebhookUpdate {
    /**
     * A new delivery endpoint. It must pass the same syntactic and literal-IP
     * checks as at creation; hostname resolution is checked at delivery time.
     * 
     */
    url?: string;
    /**
     * Pause deliveries with `false`, resume them with `true`. A webhook
     * deactivated automatically after repeated delivery failures is re-enabled
     * this way.
     * 
     */
    isActive?: boolean;
    /**
     * Replace all template-scoped match rules. Rules are ORed; within one rule
     * the template, action and signer restrictions are ANDed. The array cannot
     * be empty, and each template may appear once.
     * 
     */
...
export function WebhookUpdateFromJSONTyped(json: any, ignoreDiscriminator: boolean): WebhookUpdate {
    ...
    return {
        ...
        'rules': json['rules'] == null ? undefined : ((json['rules'] as Array<any>).map(WebhookRuleFromJSON)),
    };
}
...
export function WebhookUpdateToJSONTyped(value?: WebhookUpdate | null, ignoreDiscriminator: boolean = false): any {
    ...
    return {
        ...
        'rules': value['rules'] == null ? undefined : ((value['rules'] as Array<any>).map(WebhookRuleToJSON)),
    };
}

...
6. Public object pages call `GET /public/objects/{objectId}` from the server and adapt only the `PublicSmartObject` projection; they never fall back to the authenticated object response.
